...
import com.mlearn.cnn.dataset.Dataset.Record;
import com.mlearn.cnn.util.ConcurenceRunner.TaskManager;
import com.mlearn.cnn.util.Log;
import com.mlearn.cnn.util.Util;
import com.mlearn.cnn.util.Util.Operator;
...
public class CNN implements Serializable {
				...
				for (int i = start; i < end; i++) {
					...
					for (int j = 0; j < nextMapNum; j++) {
						...
						if (sum == null)
							...
							sum = Util.matrixOp(Util.convnFull(nextError,
											Util.rot180(kernel)), sum, null,null, Util.plus);
					}
			...
			public void process(int start, int end) {
				for (int m = start; m < end; m++) {
					...
					double[][] outMatrix = Util.matrixOp(map,
							Util.cloneMatrix(map), null, Util.one_value, Util.multiply);
					outMatrix = Util.matrixOp(outMatrix,
							Util.kronecker(nextError, scale), null, null, Util.multiply);
	...
	public void predict(Dataset testset, String fileName) {
		...
		try {
			...
			System.out.println(right + "/" + count + "=" + (float)right/count);
			writer.flush();
			writer.close();
		} catch (IOException e) {
