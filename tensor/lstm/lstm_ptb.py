...
# 数据参数
DATA_PATH = '/Users/xianling.chen/Documents/t2t_datagen/data/simple-examples/data'  # 数据存放路径
...
def restore_model_ckpt():
    #     premodel = PTBModel(False, EVAL_BATCH_SIZE, EVAL_NUM_STEP)
    #     #
    #     saver = tf.train.Saver()
    #     with tf.Session() as sess:
    #         # 参数恢复
    ...
    #         for i in range(100):
    #             onehot_pred = sess.run(premodel.logits, feed_dict={premodel.input_data: [[12]]})
    #             onehot_pred_index = int(tf.argmax(onehot_pred, 1).eval())
    #             predicts.append(onehot_pred_index)
    ...
    # # print(sess.run('w:0'))
    #
    # input_x = sess.graph.get_tensor_by_name('language_model/Placeholder:0')
    # # # 获取需要进行计算的operator
    # op = sess.graph.get_tensor_by_name('language_model_1/add:0')
    # print(sess.run(op, feed_dict={input_x: [[12]]}))
    # sess.close()
...
def print_variables():
    reader = pywrap_tensorflow.NewCheckpointReader('save_path/model')
...
if __name__ == '__main__':
    tf.app.run()
    # train_data, valid_data, test_data, _ = reader.ptb_raw_data('/Users/xianling.chen/PycharmProjects/AI/tensor/datas')
    # print()
    # print_variables()
    # restore_model_ckpt()
