...
flags.DEFINE_string("data_path", '/Users/xianling.chen/Documents/t2t_datagen/data/simple-examples/data', "data_path")
...
def run_epoch(session, model, data, eval_op, epoch_size, verbose=False):
    """Runs the model on the given data."""
    # epoch_size 表示批次总数。也就是说，需要向session喂这么多次数据
    # epoch_size = ((len(data) // model.batch_size) - 1) // model.num_steps
    # epoch_size = data.batch_count
    start_time = time.time()
    costs = 0.0
    iters = 0
    state = session.run(model.initial_state)
    # train_queue = reader.ptb_producer(data, model.batch_size, model.num_steps)
    for step in range(epoch_size):
        # _batch = data.next()
        # x = _batch[:model.num_steps]
        # y = _batch[1:]
...
if __name__ == '__main__':
    ...
    with tf.Graph().as_default(), tf.Session() as session:
        ...
        for i in range(config.max_max_epoch):  # 所有文本要重复多次进入模型训练
            ...
            train_perplexity = run_epoch(session, m, train_queue, m.train_op, train_epoch_size, verbose=True)  # 训练困惑度
            print("Epoch: %d Train Perplexity: %.3f" % (i + 1, train_perplexity))
            valid_perplexity = run_epoch(session, mvalid, valid_queue, tf.no_op(), valid_epoch_size)  # 检验困惑度
            print("Epoch: %d Valid Perplexity: %.3f" % (i + 1, valid_perplexity))

        test_perplexity = run_epoch(session, mtest, test_queue, tf.no_op(), test_epoch_size)  # 测试困惑度
